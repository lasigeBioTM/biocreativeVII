...
os.environ["CUDA_VISIBLE_DEVICES"]="0, 3, 4, 5, 6, 7, 8"
...
from ontology_preprocessing import *
from parse_biocreative_vii import sst_light_directory
import models
...
data_directory = 'data'
temporary_directory = 'temp_test/' # temp_sample/
...
# --------------------------------------------------------------
'''
def get_xml_data(base_dir, name_to_id, synonym_to_id):
    ...
    from parse_biocreative_vii import support_sdp
    parser = 'spacy'
    left_instances = manager.list([]) # word indexes
    right_instances = manager.list([])
    left_ancestors = manager.list([])
    right_ancestors = manager.list([])
    common_ancestors = manager.list([]) # ontology IDs (shared = common) 
    left_wordnet = manager.list([])  # wordnet IDs
    right_wordnet = manager.list([])
    classes = manager.list([])
    labels = manager.list([])
    all_pos_gv = manager.list([])  # anti positive governors
    all_neg_gv = manager.list([])

    dir_labels, dir_instances, dir_classes, dir_common, dir_ancestors, dir_wordnet, \
    neg_gv, pos_gv = support_sdp(base_dir, name_to_id, synonym_to_id, parser, left_instances, right_instances, left_ancestors, right_ancestors, 
    common_ancestors, left_wordnet, right_wordnet, classes, labels, all_pos_gv, all_neg_gv)
    ...
    classes = np.concatenate((classes, np.array(dir_classes)), axis = 0)

    all_pos_gv += pos_gv
    all_neg_gv += neg_gv

    return labels, (left_instances, right_instances), classes, common_ancestors, (left_ancestors, right_ancestors), (left_wordnet, right_wordnet)
'''
...
def join_channels(model_name, pair_type, channels, y_train, train_labels, n_classes, x_words_train, x_wordnet_train, x_subpaths_train, x_ancestors_train, id_to_index):
    ...
    label_to_pair_type = {'NO_RELATION': 0, 'INHIBITOR': 1, 'PART-OF': 2, 'SUBSTRATE': 3, 'ACTIVATOR': 4,
                          ...
                          'ANTAGONIST': 6, 'INDIRECT-UPREGULATOR': 7, 'AGONIST': 8, 'DIRECT-REGULATOR': 9,
                          'PRODUCT-OF': 10,
                          'AGONIST-ACTIVATOR': 11, 'AGONIST-INHIBITOR': 12, 'SUBSTRATE_PRODUCT-OF': 13}


    class_weight = {0: 0.5, 1: 1.0, 2: 1.7670781432557048, 3: 1.0, 4: 1.2890649159309184, 5: 1.3608608726449865,
                    6: 1.6744392894003468, 7: 1.3246812160674841, 8: 2.0630715593582787, 9: 1.0, 10: 1.728335057605479,
                    11: 5.186499263874312, 12: 5.9888457363992496, 13: 5.334919268992585}
...
def main():
    ...
    if type_of_action == 'preprocess':
        ...
        train_labels, x_train, y_train, x_train_ancestors, x_train_subpaths, x_train_wordnet = get_xml_data(corpora_directory)
        #print(train_labels, x_train, y_train, x_train_ancestors, x_train_subpaths, x_train_wordnet)
